...
int read_adc(void){
	...
	while(ADC_GetFlagStatus(ADC1,ADC_SR_EOC)==RESET);
	ai=ADC_GetConversionValue(ADC1);
	return (ai);
}
static void prvSetupHardware( void )
{
...
ADC_InitTypeDef potInit;
EXTI_InitTypeDef EXTI_InitStructure;
NVIC_InitTypeDef NVIC_InitStructure;
NVIC_InitTypeDef NVIC_Str;
GPIO_InitTypeDef USpinInit;
...
void SystemCoreClockUpdate( void );
  ...
  RCC_HSICmd(ENABLE);
	...
	RCC_APB1PeriphClockCmd( RCC_APB1Periph_COMP|RCC_APB1Periph_USART2, ENABLE );
	...
	NVIC_Init( &NVIC_InitStructure );
	        ...
	        /* NVIC Enabe for USART2 */

        NVIC_Str.NVIC_IRQChannel = USART2_IRQn;
...
USpinInit.GPIO_Pin = GPIO_Pin_2|GPIO_Pin_3 ;    // configure pins 9 and 10
USpinInit.GPIO_Mode = GPIO_Mode_AF; 	        // GPIO pin as output
USpinInit.GPIO_Speed = GPIO_Speed_2MHz;		// GPIO Speed 
USpinInit.GPIO_OType = GPIO_OType_PP;		// Output type as pushpull
USpinInit.GPIO_PuPd = GPIO_PuPd_UP;		// output Pull up
GPIO_Init(GPIOA, &USpinInit);
...
/* USART2 Configuration*/
  ...
  USART_Cmd(USART2, ENABLE);
	...
	adcpinInit.GPIO_Pin=GPIO_Pin_0;
	adcpinInit.GPIO_Mode=GPIO_Mode_AN;
	GPIO_Init(GPIOC, &adcpinInit);
	potInit.ADC_Resolution=ADC_Resolution_8b;
	potInit.ADC_NbrOfConversion=1;
	potInit.ADC_ContinuousConvMode=DISABLE;
	
	ADC_Init(ADC1,&potInit);
	
	ADC_Cmd(ADC1,ENABLE);
	ADC_RegularChannelConfig(ADC1,ADC_Channel_10,1,ADC_SampleTime_48Cycles);
